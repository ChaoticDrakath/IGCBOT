...
client = Bot(description="IGC Bot Bot is best", command_prefix="igc!", pm_help = True)
client.remove_command('help')
 

async def status_task():
    while True:
        await client.change_presence(game=discord.Game(name='for igc!help'))
...
async def on_member_join(member):
    ...
    channel = discord.utils.get(client.get_all_channels(), server__name='【I.G.C】Indian Gaming Community', name='IGC BOT-servers-join-leave-log')
    r, g, b = tuple(int(x * 255) for x in colorsys.hsv_to_rgb(random.random(), 1, 1))
    embed = discord.Embed(title=f'Welcome {member.name} to {member.server.name}', description='Do not forget to check <#rules-info> and never try to break any one of them', color = discord.Color((r << 16) + (g << 8) + b))
...
async def iamdark(ctx):
    author = ctx.message.author
    await client.delete_message(ctx.message)
    role = discord.utils.get(ctx.message.server.roles, name='Sunny Singh')
...
async def help(ctx):
    ...
    embed.add_field(name = '``Our Help Server Link`` ',value ='https://discordapp.com/invite/ASS4xj',inline = False)
    embed.add_field(name = 'igc!modhelp ',value ='Explaines all the commands which are only usable by Those who has moderation permissions. Like- Manage Nicknames, Manage Messages, Kick/Ban Members,etc.',inline = False)
    embed.add_field(name = 'igc!generalhelp ',value ='Explaines all the commands which are usable by everyone.',inline = False)
    await client.send_message(author,embed=embed)
...
async def welcomedbs(ctx, *, msg = None):
    ...
    else: await client.say('Welcome' + msg +  ', Please check #rules-info and never try to break any one of them')
    return
